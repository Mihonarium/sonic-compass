...
// This approach adds interaural delays, a mild front/back filter and softened
// level differences to create a more pleasant spatial tone.
const directionalBuffer = (freq, durSec, angleDeg = 0) => {
  ...
  // Equal-power panning then blended for reduced ILD
  let rawLeft = Math.cos((pan + 1) * Math.PI / 4);
  let rawRight = Math.sin((pan + 1) * Math.PI / 4);
  const BLEND = 0.15; // how much of opposite channel to mix
  const leftGain = rawLeft * (1 - BLEND) + rawRight * BLEND;
  const rightGain = rawRight * (1 - BLEND) + rawLeft * BLEND;

  // Angle-based front/back attenuation (slightly muffled when behind)
  const fbGain = 0.8 + 0.2 * fb; // 1 front, 0.6 back, 0.8 sides
  ...
  // Slightly soften high frequencies when sound comes from behind
  if (fb < 0) {
    let prev = base[0];
    const SMOOTH = 0.5;
    for (let i = 1; i < frames; i++) {
      const smoothed = prev + SMOOTH * (base[i] - prev);
      base[i] = smoothed;
      prev = smoothed;
    }
  }
